...
const TicketDetailPage = () => {
  ...
  const { id } = useParams();
  const { selectedTicket, loading, fetchTicketById, addReply, closeTicket } =
    useTicketStore();
            ...
            {selectedTicket.replies.length === 0 ? (
              <div className="bg-white dark:bg-gray-800 rounded-lg p-6 text-center shadow-sm">
                <p className="text-gray-500 dark:text-gray-400 text-sm">
                  Belum ada balasan. Silakan kirim pesan untuk memulai
                  percakapan.
                </p>
              </div>
            ) : (
              selectedTicket.replies.map((reply) => (
                <div
                  key={reply.id}
                  className={`flex ${
                    reply.sender === "user" ? "justify-end" : "justify-start"
                  }`}
                >
                  <div
