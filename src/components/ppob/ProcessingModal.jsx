...
const ProcessingModal = ({ isOpen, status, data, onClose }) => {
  ...
  const statusConfig = {
    ...
    failed: {
      ...
      description:
        data?.failReason || "Terjadi kesalahan saat memproses transaksi",
      color: "red",
    },
  };
          ...
          {status !== "processing" && (
            <div className="flex gap-3">
              ...
              <button
                onClick={onClose}
                className={`flex-1 ${
                  status === "success" ? "bg-[var(--c-primary)]" : "bg-gray-600"
                } text-white rounded-xl py-3 font-semibold hover:opacity-90 transition-opacity`}
              >
