...
const Header = () => {
  ...
  const BANK_INFORMATION = useMemo(() => {
    return {
      bank_name: user?.business_account?.bank_information?.bank_name,
      account_number: user?.business_account?.bank_information?.account_number,
    };
  }, [user?.business_account?.bank_information]);
  ...
  const renderElements = useMemo(() => {
    return (
      <header className="flex flex-col relative mb-3">
        ...
        <p className="flex text-xs ml-[64px] absolute top-[54px]">
          {user?.business_account !== null ? (
            <div className="relative">
              <span>{BANK_INFORMATION?.bank_name || "Belum ada rekening"}</span>
              :{" "}
              <span className="font-semibold">
                {BANK_INFORMATION?.account_number || "Belum ada rekening"}
              </span>
            </div>
          ) : (
            <span className="text-red-500">Belum ada rekening</span>
          )}
        </p>
      </header>
    );
